...
import { ExecuteActionComponent } from './engine/ecs/components/index.mjs';
import { AISystem } from './engine/ecs/systems/aiSystem.mjs';
import { InteractionSystem } from './engine/ecs/systems/interactionSystem.mjs';
import { InterruptSystem } from './engine/ecs/systems/interruptSystem.mjs';
import { MovementSystem } from './engine/ecs/systems/movementSystem.mjs';
import { RenderSystem } from './engine/ecs/systems/renderSystem.mjs';
import { StatCalculationSystem } from './engine/ecs/systems/statCalculationSystem.mjs';
import { World } from './engine/ecs/world.mjs';
...
import { InteractionManager } from './engine/interactionManager.mjs';
import { MapManager } from './engine/mapManager.mjs';
import { ModifierManager } from './engine/modifierManager.mjs';
import { PlayerTurnController } from './engine/playerTurnController.mjs';
import { Renderer } from './engine/renderer.mjs';
import { ScriptingService } from './engine/scriptingService.mjs';
import { globalServiceLocator } from './engine/serviceLocator.mjs';
import { TargetingManager } from './engine/targetingManager.mjs';
import { TileStateManager } from './engine/tileStateManager.mjs';
...
async function initializeGame(winObj: any) {
    ...
    globalServiceLocator.contentLoader = contentLoader;

    const modifierTypes = await contentLoader.loadModifierTypes();
    const attributeDefinitions = await contentLoader.loadAttributeDefinitions();
    const modifierManager = new ModifierManager(modifierTypes, attributeDefinitions);
    globalServiceLocator.modifierManager = modifierManager;
    ...
    globalServiceLocator.tileStateManager = new TileStateManager();
    globalServiceLocator.interactionManager = new InteractionManager();
    globalServiceLocator.world = new World();
    globalServiceLocator.scriptingService = new ScriptingService();
    globalServiceLocator.targetingManager = new TargetingManager();
    globalServiceLocator.aiSystem = new AISystem();
    globalServiceLocator.interruptSystem = new InterruptSystem();
    globalServiceLocator.renderSystem = new RenderSystem();
    globalServiceLocator.movementSystem = new MovementSystem();
    globalServiceLocator.interactionSystem = new InteractionSystem();
    globalServiceLocator.statCalculationSystem = new StatCalculationSystem(modifierManager);
    globalServiceLocator.mapManager = new MapManager();
    globalServiceLocator.game = new Game();
    ...
    globalServiceLocator.eventBus.subscribe('ui:creation:next_step', () => {
        ...
        if (currentStepId === 'abilityScoreSelection') {
            ...
            globalServiceLocator.statCalculationSystem.recalculateStats(globalServiceLocator.state.playerId!, globalServiceLocator.world);
        } else if (currentStepId === 'classSelection' && globalServiceLocator.state.playerId) {
            globalServiceLocator.statCalculationSystem.recalculateStats(globalServiceLocator.state.playerId, globalServiceLocator.world);
        }
    ...
    globalServiceLocator.eventBus.subscribe(GameEvents.UI_CREATION_CONFIRMED, async () => {
        ...
        if (!state.playerId) {
            console.error("Cannot start game: Player ID is missing.");
            return;
        }

        if (!state.currentCampaignData) {
            console.error("Cannot start game: Campaign data is missing.");
            return;
        }
        ...
        // 1. Load the starting map data from the selected campaign.
        await globalServiceLocator.mapManager.loadMap("starting_area");
        ...
        globalServiceLocator.game.start();
    });
    ...
    globalServiceLocator.renderer.canvas.addEventListener('click', (event: MouseEvent) => {
        const targetingManager = globalServiceLocator.targetingManager;
        if (!targetingManager.isTargeting) {
            return;
        }

        const rect = globalServiceLocator.renderer.canvas.getBoundingClientRect();
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top;

        const tileX = Math.floor(x / globalServiceLocator.renderer.tileSize);
        const tileY = Math.floor(y / globalServiceLocator.renderer.tileSize);

        // In a real implementation, we would check if there is a valid target at this tile.
        // For now, we'll just assume the first NPC is the target.
        const targetId = globalServiceLocator.state.npcs[0];
        const playerId = globalServiceLocator.state.playerId;

        if (targetId !== null && playerId !== null && targetingManager.action) {
            const action = targetingManager.action;
            action.target = targetId;
            globalServiceLocator.world.addComponent(playerId, new ExecuteActionComponent(action));
        }

        targetingManager.stopTargeting();
    });
    updateUI();
...
function getUiScreens(winDoc: Document): UIHolder {
    return {
        els: {
            ...
            'featSelectionView': winDoc.getElementById('featSelectionView') as HTMLElement,
            'featSlots': winDoc.getElementById('featSlots') as HTMLElement,
            'availableFeats': winDoc.getElementById('availableFeats') as HTMLElement,
        },
        inputs: {
        ...
        btns: {
            ...
            'confirmFeatButton': winDoc.getElementById('confirmFeatButton') as HTMLButtonElement,
        }
    } as UIHolder;
};
